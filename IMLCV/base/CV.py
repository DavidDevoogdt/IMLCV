...
class CV:
    ...
    def __init__(self, f, **kwargs) -> None:
        """
        f
            function  f(coordinates, cell, **kwargs) that returns the CV
        """
        self.f = f
        self.kwargs = kwargs
    ...
    def compute(self, coordinates, cell, grad=None, vir=None):
        """
        coodinates 
            cartesian coordinates, as numpy array of form (number of atoms,3)
        cell
            cartesian coordinates of cell vectors, as numpy array of form (3,3)
        grad
            if not None, is set to the to gradient of CV wrt coordinates
        vir
