...
def cv_norm(x: CV, y: CV, metric: Metric):
    # return metric.norm(x, y)
    return jnp.linalg.norm(x.cv - y.cv)


def cv_vals(x: CV, metric: Metric):
    return x.cv  # metric.min_cv(x.cv)
...
def polynomial_vector(x: CV, powers, metric: Metric, shift, scale):
    """Evaluate monomials, with exponents from `powers`, at the point `x`."""

    g = lambda x: (cv_vals(x, metric=metric) - shift) / scale
...
def polynomial_matrix(x: CV, shift, scale, metric: Metric, powers):
...
def _polynomial_matrix(x: CV, powers, metric, shift, scale):
    """Return monomials, with exponents from `powers`, evaluated at `x`."""
    assert isinstance(x, jnp.ndarray)

    out = polynomial_matrix(x=x, metric=metric, scale=scale, shift=shift, powers=powers)
    return out
...
def _build_system(y: CV, metric: Metric, d, smoothing, kernel, epsilon, powers):
    ...
    P = polynomial_matrix(y, shift=shift, scale=scale, metric=metric, powers=powers)
    lhs = jnp.block([[K, P], [P.T, jnp.zeros((P.shape[1], P.shape[1]))]])

    # lhs = jnp.empty((p + r, p + r), dtype=float).T
    # lhs = lhs.at[:p, :p].set(kernel_matrix(y, metric,epsilon,kernel_func))
    # lhs = lhs.at[:p, p:].set(polynomial_matrix(yhat, powers))
    # lhs = lhs.at[p:, :p].set(lhs[:p, p:].T)
    # lhs = lhs.at[p:, p:].set( 0.0)
    # for i in range(p):
    #     lhs = lhs.at[i, i].set( lhs[i, i] + smoothing[i])

    # Transpose to make the array fortran contiguous.
    rhs = jnp.vstack([d, jnp.zeros((r, s))])

    # rhs = jnp.empty((s, p + r), dtype=float).T
    # rhs = rhs.at[:p].set(d)
    # rhs = rhs.at[p:].set(0.0)
...
def _build_evaluation_coefficients(
    x: CV, y: CV, metric: Metric, kernel, epsilon, powers, shift, scale
):
    ...
    pv = lambda x: polynomial_vector(x, powers, metric=metric, scale=scale, shift=shift)
    pv0 = jax.vmap(pv)(x)
