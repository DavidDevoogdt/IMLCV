...
class SOAP(Descriptor):
    ...
    def __init__(
        ...
        for var_new in ["r_cut", "n_max", "l_max"]:
            ...
            if loc.get(var_old) is not None:
                ...
                if loc.get(var_new) is not None:
                    raise ValueError(
                        "Please provide only either {} or {}.".format(
                            var_new, var_old)
                    )
            else:
        ...
        if n_max < 1:
            raise ValueError(
                "Must have at least one radial basis function." "n_max={}".format(
                    n_max)
            )
            ...
            if func is not None:
                ...
                if func not in weighting_functions:
                    raise ValueError(
                        ...
                        "among one of the following {}".format(
                            weighting_functions)
                    )
                r0 = weighting.get("r0")
                ...
                if func == "poly":
                    ...
                    if m is None or m < 0:
                        raise ValueError("Define m >= 0 in weighting.")
                    weighting["m"] = float(m)
                    weighting["threshold"] = float(
                        weighting.get("threshold", 1e-2))
                elif func == "exp":
                    d = weighting.get("d")
                    if d < 0:
                        raise ValueError("Define d >= 0 in weighting.")
        ...
        if l_max < 0:
            raise ValueError(
                "l_max cannot be negative. l_max={}".format(l_max))
        elif l_max > 20:
            raise ValueError(
    ...
    def derivatives(
        ...
        if method not in methods:
            raise ValueError(
                "Invalid method specified. Please choose from: {}".format(
                    methods)
            )
    ...
    def get_location(self, species):
        ...
        for specie in species:
            if isinstance(specie, str):
                try:
                    ...
                    raise ValueError(
                        "Invalid chemical species: {}".format(specie))
        ...
        for number in numbers:
            if number not in self._atomic_number_set:
                raise ValueError(
                    "Atomic number {} was not specified in the species.".format(
                        number)
                )
        ...
        i = numbers[0]
        j = numbers[1]
        n_elem_feat_symm = self._n_max * \
            (self._n_max + 1) / 2 * (self._l_max + 1)
        ...
        if self.crossover:
            ...
            start = int(m_symm * n_elem_feat_symm +
                        m_unsymm * n_elem_feat_unsymm)
